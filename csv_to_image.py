...
def csv_to_image(csv_file_path, output_image_path, width, height, max_entries=6700, high_condition="1"):
    ...
    with open(csv_file_path, 'r') as csv_file:
        ...
        for idx, row in enumerate(csv_reader):
            if len(row) > 2 and row[2].strip() == high_condition:  # Check if column C is high
...
csv_to_image("image_data.csv", "reconstructed_image_csv.jpg", width=100, height=67)
